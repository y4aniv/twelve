...
document.getElementById("builder-close").addEventListener("click", function () {
  ...
  fetch("/api/cocktail", {
    ...
    .then(function (data) {
      ...
      document.querySelector("#qrcode img").src =
        `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${window.location.origin}/cocktail/${data.id}&bgcolor=fdfaf6&color=0d0700`; // Générer un code QR avec l'URL du cocktail
      gsap.to(".step-2", {
