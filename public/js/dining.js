    ...
    .then((data) => {
      if (data.ingredients) {
        data.ingredients.forEach((ingredient) => {
          if (
            cocktailIngredients.includes(ingredient.strIngredient) === false
          ) {
        ...
        document
          ...
          .scrollTo(0, document.querySelector(".cocktail__labs").scrollHeight);
      }
    });
}
  ...
  fetch("/api/cocktail", {
    ...
    .then((json) => {
      document.getElementById("cocktail-qr").src =
        `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${window.location.origin}/cocktail/${json.id}`;
