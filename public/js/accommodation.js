function filterAccommodation(type) {
  // Fonction pour filtrer les types de logements
  /**
  ...
  gsap.to('.content__item[style*="display: block;"]', {
    // Animation de transition des logements pour les cacher
    ...
    onComplete() {
      ...
      elements.forEach((element) => {
        ...
        if (type === 'all' || dataType === type) {
          // Si le type est "all" ou si le type correspond
      ...
      gsap.to('.content__item[style*="display: block;"]', {
        // Animation de transition des logements pour les afficher
...
gsap.to('.content__item', {
  // Animation de transition des logements
  scrollTrigger: {
